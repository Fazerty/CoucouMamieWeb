...
export class ArticleService {
  ...
  async findFeed(userId: number, query: { limit: number; offset: number; }): Promise<ArticlesRO> {
    const _follows = await this.profileRepository.find( {followerId: userId});
  ...
  async findOne(where: unknown): Promise<ArticleRO> {
    const article = await this.articleRepository.findOne(where);
    return {article};
  }

  async addComment(slug: string, commentData: unknown): Promise<ArticleRO> {
    let article = await this.articleRepository.findOne({slug});

    const comment = new CommentEntity();
    comment.body = (commentData as any).body;
  ...
  async update(slug: string, articleData: unknown): Promise<ArticleRO> {
