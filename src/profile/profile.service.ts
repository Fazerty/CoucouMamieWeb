...
export class ProfileService {
  ...
  async findProfile(id: number, followingUsername: string): Promise<ProfileRO> {
    ...
    const follows = await this.profileRepository.findOne( {followerId: id, followingId: _profile.id});
  ...
  async follow(followerEmail: string, username: string): Promise<ProfileRO> {
    ...
    const _follows = await this.profileRepository.findOne( {followerId: followerUser.id, followingId: followingUser.id});

    if (!_follows) {
      const follows = new ProfileEntity();
      follows.followerId = followerUser.id;
      follows.followingId = followingUser.id;
      await this.profileRepository.save(follows);
    }
  ...
  async unFollow(followerId: number, username: string): Promise<ProfileRO> {
    ...
    await this.profileRepository.delete({followerId, followingId});
